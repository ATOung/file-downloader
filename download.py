...
class dl:
    ...
    def hxfile(self):
        ...
        try:
            ...
            d=f"op={tmplh[0]}&id={tmplh[1]}&rand={tmplh[2]}&referer=&method_free&method_premium=1"
            self.u=r.post(self.url,headers={"User-Agent":ua.get(),"content-type":"application/x-www-form-urlencoded"},data=d).text
...
if __name__ == "__main__":
    ...
    if arg > 3 and sys.argv[1] == "-p":
        directdl=bool(arg > 4 and sys.argv[4] == "-grabdirectlink")
        start()
        if sys.argv[2] == "mediafire":
            dl(sys.argv[3], directdl).mediafire()
        elif sys.argv[2] == "solidfiles":
            dl(sys.argv[3], directdl).solidfiles()
        elif sys.argv[2] == "tusfiles":
            dl(sys.argv[3], directdl).tusfiles()
        elif sys.argv[2] == "anonfiles":
            dl(sys.argv[3], directdl).anonfiles()
        elif sys.argv[2] == "bayfiles":
            dl(sys.argv[3], directdl).bayfiles()
        elif sys.argv[2] == "racaty":
            dl(sys.argv[3], directdl).racaty()
        elif sys.argv[2] == "zippyshare":
            dl(sys.argv[3], directdl).zippyshare()
        elif sys.argv[2] == "hxfile":
            dl(sys.argv[3], directdl).hxfile()
        else:
